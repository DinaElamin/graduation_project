...
class GetAllStudentByClassIdCubit extends Cubit<GetAllStudentsByClassIdState> {
  ...
  GetAllStudentByClassIdCubit(this.getAllStudentsByClassIdRepo) : super(const GetAllStudentsByClassIdState.initial());

  void emitAllStudentsByClassId({required int classId}) async {
    emit(const GetAllStudentsByClassIdState.loading());
    final response = await getAllStudentsByClassIdRepo.getAllStudentsByClassIdData(classId: classId);
